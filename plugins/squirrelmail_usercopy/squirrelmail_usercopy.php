...
class squirrelmail_usercopy extends crystal_plugin
{
	...
	public function init()
	{
		...
		$ilevel = $cmail->config->get('squirrelmail_identities_level');
        if ($ilevel === null)
		    $ilevel = $cmail->config->get('identities_level', 0);
		...
		if ($cmail->task == 'login' && $this->prefs) {
			...
			if (($this->identities_level == 0 || $this->identities_level == 1) && isset($this->prefs['identities']) && $this->prefs['identities'] > 1) {
				for ($i=1; $i < $this->prefs['identities']; $i++) {
					...
					// insert identity
					$identid = $cmail->user->insert_identity($ident_data);
				}
			}
	...
	private function read_squirrel_prefs($uname)
	{
		...
		/**** File based backend ****/
		if ($cmail->config->get('squirrelmail_driver') == 'file' && ($srcdir = $cmail->config->get('squirrelmail_data_dir'))) {
			if (($hash_level = $cmail->config->get('squirrelmail_data_dir_hash_level')) > 0) 
		...
		/**** Database backend ****/
		else if ($cmail->config->get('squirrelmail_driver') == 'sql') { 
			$this->prefs = array();

			/* connect to squirrelmail database */
			$db = new crystal_mdb2($cmail->config->get('squirrelmail_dsn'));
			$db->db_connect('r'); // connect in read mode

			// $db->set_debug(true);

			/* retrieve prefs */
			$userprefs_table = $cmail->config->get('squirrelmail_userprefs_table');
			$address_table = $cmail->config->get('squirrelmail_address_table');
			$db_charset = $cmail->config->get('squirrelmail_db_charset');
			...
			while ($sql_array = $db->fetch_assoc($sql_result) ) { // fetch one row from result
				$this->prefs[$sql_array['prefkey']] = crystal_charset_convert(rtrim($sql_array['prefval']), $db_charset);
			}
			...
			// parse addres book
			while ($sql_array = $db->fetch_assoc($sql_result) ) { // fetch one row from result
				$rec['name']      = crystal_charset_convert(rtrim($sql_array['nickname']), $db_charset);
				$rec['firstname'] = crystal_charset_convert(rtrim($sql_array['firstname']), $db_charset);
				$rec['surname']   = crystal_charset_convert(rtrim($sql_array['lastname']), $db_charset);
				$rec['email']     = crystal_charset_convert(rtrim($sql_array['email']), $db_charset);
				$rec['note']      = crystal_charset_convert(rtrim($sql_array['label']), $db_charset);
