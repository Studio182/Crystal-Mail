...
function password_save($curpass, $passwd)
{
	...
	/* Connect */
	if (!$ds = ldap_connect($cmail->config->get('password_ldap_host'), $cmail->config->get('password_ldap_port'))) {
		ldap_unbind($ds);
		return PASSWORD_CONNECT_ERROR;
	}

	/* Set protocol version */	
	if (!ldap_set_option($ds, LDAP_OPT_PROTOCOL_VERSION, $cmail->config->get('password_ldap_version'))) {
		ldap_unbind($ds);
		return PASSWORD_CONNECT_ERROR;
	}
	
	/* Start TLS */
	if ($cmail->config->get('password_ldap_starttls')) {
	...
	/* Build user DN */
	if ($user_dn = $cmail->config->get('password_ldap_userDN_mask')) {
		$user_dn = ldap_simple_substitute_vars($user_dn);
	} else {
		$user_dn = ldap_simple_search_userdn($cmail, $ds);
	}
	...
	switch ($cmail->config->get('password_ldap_method')) {
		case 'admin':
			$binddn = $cmail->config->get('password_ldap_adminDN');
			$bindpw = $cmail->config->get('password_ldap_adminPW');
			break;
		case 'user':
		default:
			$binddn = $user_dn;
	...
	$passwd = ldap_simple_hash_password($passwd, $cmail->config->get('password_ldap_encodage'));
	if (!$passwd) {
		ldap_unbind($ds);
		return PASSWORD_CRYPT_ERROR;
	}
	
	$entree[$cmail->config->get('password_ldap_pwattr')] = $passwd;
 ...
 * Return the found DN
 */
function ldap_simple_search_userdn($cmail, $ds)
{
	/* Bind */
	if (!ldap_bind($ds, $cmail->config->get('password_ldap_searchDN'), $cmail->config->get('password_ldap_searchPW'))) {
		return false;
	}
	
	/* Search for the DN */
	if (!$sr = ldap_search($ds, $cmail->config->get('password_ldap_search_base'), ldap_simple_substitute_vars($cmail->config->get('password_ldap_search_filter')))) {
		return false;
	}
