/**
 * crystalmail functions for default skin interface
 */
...
show_popupmenu: function(popup, show)
{
  var obj = this.popups[popup].obj,
    ...
    ref = crystal_find_object(popup+'link');
...
searchmenu: function(show)
{
  var obj = this.popups.searchmenu.obj,
    ref = crystal_find_object('searchmenulink');
...
listmenu: function(show)
{
  var obj = this.popups.listmenu.obj,
    ref = crystal_find_object('listmenulink');
...
{
  var i, target = crystal_event.get_target(evt);

  for (i in this.popups) {
    if (this.popups[i].obj.is(':visible') && target != crystal_find_object(i+'link')
      && !this.popups[i].toggle
      && (!this.popups[i].editable || !this.target_overlaps(target, this.popups[i].id))
      && (!this.popups[i].sticky || !crystal_mouse_is_over(evt, crystal_find_object(this.popups[i].id)))
    ) {
...
{
  var element = crystal_find_object(elementid);
...
switch_preview_pane: function(elem)
{
  ...
  if (elem.checked) {
    cmail.env.contentframe = 'messagecontframe';
    ...
    if (uid = cmail.message_list.get_single_selection())
      cmail.show_message(uid, false, true);
    cmail.http_post('save-pref', '_name=preview_pane&_value=1');
  }
  else {
    prev_frm.hide();
    if (bw.ie6 || bw.ie7) {
    ...
    cmail.env.contentframe = null;
    cmail.show_contentframe(false);
    cmail.http_post('save-pref', '_name=preview_pane&_value=0');
  }
},
...
init_compose_form: function()
{
  ...
  if (cc_field && cc_field.value != '')
    cmail_ui.show_header_form('cc');

  if (bcc_field && bcc_field.value != '')
    cmail_ui.show_header_form('bcc');

  // prevent from form data loss when pressing ESC key in IE
  if (bw.ie) {
    var form = crystal_find_object('form');
    form.onkeydown = function (e) {
      if (crystal_event.get_keycode(e) == 27)
        crystal_event.cancel(e);
    };
  }

  $(window).resize(function() {
    cmail_ui.resize_compose_body();
  });
...
{
  window.setTimeout(function(){cmail_ui.resize_compose_body();}, 100);
},
...
function crystal_init_mail_ui()
{
  ...
  $('iframe').load(iframe_events)
    .contents().mouseup(function(e){cmail_ui.body_mouseup(e)});
  ...
  if (cmail.env.task == 'mail') {
    ...
    cmail.addEventListener('aftersend-attachment', 'uploadmenu', cmail_ui);
    cmail.addEventListener('aftertoggle-editor', 'resize_compose_body_ev', cmail_ui);
    cmail.gui_object('message_dragmenu', 'dragmessagemenu');

    if (cmail.env.action == 'compose')
      cmail_ui.init_compose_form();
  }
}
