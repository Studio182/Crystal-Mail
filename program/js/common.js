...
/**
 * cmail objects event interface
 */
function crystal_event_engine()
{
  this._events = {};
};

crystal_event_engine.prototype = {
...
};  // end crystal_event_engine.prototype



/**
 * crystalmail generic layer (floating box) class
 *
 * @constructor
 */
function crystal_layer(id, attributes)
{
  this.name = id;
...
function crystal_check_email(input, inline)
{
...
function crystal_clone_object(obj)
{
  var out = {};
...
function crystal_find_object(id, d)
{
  var n, f, obj, e;
  if(!d) d = document;
  ...
  if (!obj && d.layers) {
    ...
    for (n=0; !obj && n<d.layers.length; n++)
      obj = crystal_find_object(id, d.layers[n].document);
  }
...
function crystal_mouse_is_over(ev, obj)
{
  var mouse = crystal_event.get_mouse_pos(ev);
  var pos = $(obj).offset();
...
crystalmail_browser.prototype.set_cookie = setCookie;
crystalmail_browser.prototype.get_cookie = getCookie;

// tiny replacement for Firebox functionality
function crystal_console()
{
  this.log = function(msg)
  {
    var box = crystal_find_object('dbgconsole');
...
var bw = new crystalmail_browser();
bw.set_html_class();

if (!window.console) 
  console = new crystal_console();
