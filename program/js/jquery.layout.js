 ...
 */
	...
	var initPanes = function () {
		...
		$.each(_c.allPanes.split(","), function (idx, pane) {
			...
			if (!$P.data("layoutCSS")) {
				var props = "position,top,left,bottom,right,width,height,overflow,zIndex,display,backgroundColor,padding,margin";
		...
		$.each(panes.split(","), function (i, pane) {
			...
			if (pane == "center") {
				...
				if (!hasRoom && !state.initialized && o.minWidth > 0) {
					...
					// IF we found enough extra space, then resize the  panes as calculated
					if (reqPx == 0) {
						if (sizeE != minE)
