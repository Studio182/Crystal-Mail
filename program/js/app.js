/*
 ...
 |                                                                       |
 +-----------------------------------------------------------------------+
 | Authors: Thomas Bruederli <crystalmail@gmail.com>                       |
...
function crystal_webmail()
{
  ...
  $.ajaxSetup({
    ...
    beforeSend:function(xmlhttp){ xmlhttp.setRequestHeader('X-crystalmail-Request', ref.env.request_token); }
  });
  ...
  this.init = function()
  {
    ...
    for (var n in this.gui_objects)
      this.gui_objects[n] = crystal_find_object(this.gui_objects[n]);
    ...
    switch (this.task) {
      ...
      case 'mail':
        ...
        if (this.gui_objects.messagelist) {
          ...
          this.message_list = new crystal_list_widget(this.gui_objects.messagelist, {
        ...
        if (this.gui_objects.qsearchbox) {
          ...
          $(this.gui_objects.qsearchbox).focusin(function() { cmail.message_list.blur(); });
        }
      ...
      case 'addressbook':
        ...
        if (this.gui_objects.contactslist) {
          ...
          this.contact_list = new crystal_list_widget(this.gui_objects.contactslist,
          ...
          document.onmouseup = function(e){ return p.doc_mouse_up(e); };
          if (this.gui_objects.qsearchbox) {
            $(this.gui_objects.qsearchbox).focusin(function() { cmail.contact_list.blur(); });
          }
        }
        ...
        if (this.env.cid)
          this.enable_command('abooknav', 'show', 'edit', 'add-group-member', 'del-group-member', true);

        if ((this.env.action=='add' || this.env.action=='edit') && this.gui_objects.editform)
          this.enable_command('abooknav', 'add-altemail', 'save', true);
        else
      ...
      case 'settings':
        ...
        if (this.gui_objects.identitieslist) {
          this.identity_list = new crystal_list_widget(this.gui_objects.identitieslist, {multiselect:false, draggable:false, keyboard:false});
          ...
          this.sections_list = new crystal_list_widget(this.gui_objects.sectionslist, {multiselect:false, draggable:false, keyboard:false});
      ...
      case 'login':
        ...
        input_user.bind('keyup', function(e){ return cmail.login_user_keyup(e); });
        ...
        $('form').submit(function () { 
          cmail.display_message(cmail.get_label('loading'), 'loading', true); 
        }); 
  ...
  this.command = function(command, props, obj)
  {
    ...
    if (!this.commands[command]) {
      ...
      if (this.env.framed && parent.cmail && parent.cmail.command)
        parent.cmail.command(command, props);
    ...
    switch (command) {
      ...
      case 'save':
        if (this.gui_objects.editform) {
          ...
          var input_altemail = $("input[name='_altemail[]']");
          ...
          // contacts/identities
            ...
            if (input_name.length && input_name.val() == '') {
              ...
              if (this.env.task == 'addressbook')
                this.command('abooknav', 'basic');
              input_name.focus();
              break;
            }
            else if (input_email.length && !crystal_check_email(input_email.val())) {
	          ...
	          var error = false;
	          input_altemail.each(function() {
	            if ($(this).val().length && !crystal_check_email($(this).val())) {
                  alert(ref.get_label('noemailwarning'));
                  if (ref.env.task == 'addressbook')
                    ref.command('abooknav', 'basic');
                  $(this).focus();
                  error = true;
                  return false;
                }
      ...
      case 'load-attachment':
        ...
        if (this.env.uid && props.mimetype && $.inArray(props.mimetype, this.mimetypes)>=0) {
          if (props.mimetype == 'text/html')
            qstring += '&_safe=1';
          this.attachment_win = window.open(this.env.comm_path+'&_action=get&'+qstring+'&_frame=1', 'crystalmailattachment');
      ...
      case 'abooknav':
        var prev_pane = $('.tablink-selected').attr('id').replace(/addresstab/, 'addressdiv');
        $('#' + prev_pane).hide();
        $('.tablink-selected').removeClass('tablink-selected');
        $('#addresstab' + props).addClass('tablink-selected');
        $('#addressdiv' + props).show();

        if (document.getElementById('contacttab'))
          document.getElementById('contacttab').value = props;

        // hide buttons on groups tab
        $('#addressbuttons').show();
        if (props == 'groups')
          $('#addressbuttons').hide();
        break;
  ...
  this.reload = function(delay)
  {
    if (this.env.framed && parent.cmail)
      parent.cmail.reload(delay);
    else if (delay)
      window.setTimeout(function(){ cmail.reload(); }, delay);
  ...
  {
    var modkey = crystal_event.get_modifier(e),
  ...
  this.drag_move = function(e)
  {
    if (this.gui_objects.folderlist && this.env.folder_coords) {
      ...
      mouse = crystal_event.get_mouse_pos(e);
  ...
  this.doc_mouse_up = function(e)
  {
    ...
    if (this.message_list) {
      if (!crystal_mouse_is_over(e, this.message_list.list.parentNode))
      ...
      if (!crystal_mouse_is_over(e, this.contact_list.list.parentNode))
  ...
  this.add_message_row = function(uid, cols, flags, attop)
  {
    ...
    if (!bw.ie && cols.subject) {
      ...
      cols.subject = '<a href="./?_task=mail&_action='+action+'&_mbox='+urlencode(flags.mbox)+'&'+uid_param+'='+uid+'"'+
        ' onclick="return crystal_event.cancel(event)">'+cols.subject+'</a>';
    }
  ...
  {
    var key = crystal_event.get_keycode(e);
    var passwd = $('#rcmloginpwd');

    // enter
    if (key == 13 && passwd.length && !passwd.val()) {
      passwd.focus();
      return crystal_event.cancel(e);
    }
  ...
  this.check_compose_input = function()
  {
    ...
    if (input_from.attr('type') == 'text' && !crystal_check_email(input_from.val(), true)) {
    ...
    if (!crystal_check_email(recipients.replace(/^\s+/, '').replace(/[\s,;]+$/, ''), true)) {
  ...
  this.upload_file = function(form)
  {
    ...
    if (send) {
      ...
      $(frame_name).bind('load', {ts:ts}, function(e) {
        ...
        if (!content.match(/add2attachment/) && (!bw.opera || (cmail.env.uploadframe && cmail.env.uploadframe == e.data.ts))) {
      ...
      if (this.env.cancelicon)
        content = '<a title="'+this.get_label('cancel')+'" onclick="return cmail.cancel_attachment_upload(\''+ts+'\', \''+frame_name+'\');" href="#cancelupload"><img src="'+this.env.cancelicon+'" alt="" /></a>'+content;
  ...
  this.ksearch_keypress = function(e, obj)
  {
    ...
    var highlight;
    var key = crystal_event.get_keycode(e);
    var mod = crystal_event.get_modifier(e);
  ...
  this.load_contact = function(cid, action, framed)
  {
    ...
    // send currently selected tab
    if ($('.tablink-selected').attr('id'))
      add_url += '&_tab=' + $('.tablink-selected').attr('id').replace(/addresstab/, '');
  ...
  this.add_altemail = function()
  {
    var dataTable = crystal_find_object('rcm_basictbl').tBodies[0];
	newNode = dataTable.rows[0].cloneNode(true);
	dataTable.appendChild(newNode);
	newNode.cells[0].innerHTML = newNode.cells[0].innerHTML.replace(/_X/g, this.env.altemailcount);
	newNode.cells[1].innerHTML = newNode.cells[1].innerHTML.replace(/_X/g, this.env.altemailcount);
	newNode.style.display = "";
	this.env.altemailcount++
  }
  ...
  this.group_create = function()
  {
    ...
    if (!this.name_input) {
      ...
      this.name_input.bind('keydown', function(e){ return cmail.add_input_keydown(e); });
  ...
  this.add_group_member = function(gid)
  {
	  this.copy_contact(this.env.cid, {type: 'group', source: this.env.source, id: gid});
  }

  this.del_group_member = function(gid)
  {
	  this.http_post('group-delmembers', '_cid='+urlencode(this.env.cid)+'&_source='+urlencode(this.env.source)+'&_gid='+urlencode(gid));
  }
  ...
  this.insert_contact_group = function(prop)
  {
    ...
    var link = $('<a>').attr('href', '#')
      .bind('click', function() { return cmail.command('listgroup', prop, this);})
      .html(prop.name);
  ...
  {
    var p = this;
    this.subscription_list = new crystal_list_widget(this.gui_objects.subscriptionlist, {multiselect:false, draggable:true, keyboard:false, toggleselect:true});
  ...
  this.rename_folder = function(id)
  {
    ...
    if (id && this.env.subscriptionrows[id] && (row = document.getElementById(id))) {
      ...
      this.name_input.onkeydown = function(e){ cmail.name_input_keydown(e); };
  ...
  this.init_buttons = function()
  {
    for (var cmd in this.buttons) {
      ...
      for (var i=0; i< this.buttons[cmd].length; i++) {
        ...
        elm._id = prop.id;
        if (prop.sel) {
          elm.onmousedown = function(e){ return cmail.button_sel(this._command, this._id); };
          elm.onmouseup = function(e){ return cmail.button_out(this._command, this._id); };
          if (preload)
            new Image().src = prop.sel;
        }
        if (prop.over) {
          elm.onmouseover = function(e){ return cmail.button_over(this._command, this._id); };
          elm.onmouseout = function(e){ return cmail.button_out(this._command, this._id); };
          if (preload)
      ...
      for (n=0, len=this.env.coltypes.length; n<len; n++) {
        ...
        if ((cell = thead.rows[0].cells[n]) && (col=='from' || col=='to')) {
          ...
          if (cell.firstChild && cell.firstChild.tagName.toLowerCase()=='a') {
            ...
            cell.onclick = function(){ return cmail.command('sort', this.__col, this); };
            cell.__col = col;
          }
  ...
  this.load_headers = function(elem)
  {
    ...
    elem.onclick = function() { cmail.hide_headers(elem); };
  ...
  this.hide_headers = function(elem)
  {
    ...
    elem.onclick = function() { cmail.load_headers(elem); };
  };
  ...
  this.html2plain = function(htmlText, id)
  {
    ...
    $.ajax({ type: 'POST', url: url, data: htmlText, contentType: 'application/octet-stream',
      error: function(o) { cmail.http_error(o); },
      success: function(data) { cmail.set_busy(false); $(document.getElementById(id)).val(data); console.log(data); }
    });
  };
...
}  // end object crystal_webmail

// copy event engine prototype
crystal_webmail.prototype.addEventListener = crystal_event_engine.prototype.addEventListener;
crystal_webmail.prototype.removeEventListener = crystal_event_engine.prototype.removeEventListener;
