...
class crystal_contacts extends crystal_addressbook
{
    ...
    function __construct($dbconn, $user)
    {
        ...
        $this->fieldmap = array_flip($this->table_cols);
    }
    ...
    /**
     * List all groups that a specified contact is a member of
     *
     * @param int  search contact ID
     */
    function contact_groups($contactid)
    {
        $results = array();
        ...
        $sql_result = $this->db->query(
            "SELECT * FROM ".get_table_name($this->db_groups)." AS g".
            " LEFT JOIN ".get_table_name($this->db_groupmembers)." AS m".
            " ON (m.contactgroup_id = g.contactgroup_id)".
            " WHERE del<>1".
            " AND user_id=?".
            " AND contact_id=?".
            " ORDER BY name",
   ...
   * @param int     ID of the address book instance
   * @return array with 'email' and 'name'
   */
  function auto_complete($search, $max, $id)
  {
	$contacts = array();
    ...
    while ($result && ($sql_arr = $result->iterate())) {
      ...
      if ($sql_arr['vcard']) {
        ...
        for ($i = 1; $i < sizeof($vcard->email); $i++)
          $contacts[] = format_email_recipient($vcard->email[$i], $sql_arr['name']);
      }
    ...
    foreach ($this->list_groups($search) as $group) {
      $this->reset();
