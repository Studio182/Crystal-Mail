...
/*
 ...
 | Copyright (C) 2008-2010, crystalmail Dev. - Switzerland                 |
...
class cmail
{
  ...
  function login($username, $pass, $host=NULL)
  {
    ...
    else if (!empty($config['default_host']) && $host != crystal_parse_host($config['default_host']))
      return false;
    ...
    */
    ...
    if (!empty($config['username_domain']) && !strpos($username, '@')) {
      if (is_array($config['username_domain']) && isset($config['username_domain'][$host]))
        $username .= '@'.crystal_parse_host($config['username_domain'][$host]);
      else if (is_string($config['username_domain']))
        $username .= '@'.crystal_parse_host($config['username_domain']);
    }
  ...
  public function autoselect_host()
  {
    ...
    if (is_array($default_host)) {
      ...
      $host = crystal_parse_host($default_host);
  ...
  /**
   * Build a valid URL to this instance of crystalmail
   *
