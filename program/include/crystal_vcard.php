...
/*
 ...
 | Copyright (C) 2008-2009, Crystal Mail Dev. - United States                 |
...
class crystal_vcard
{
  ...
  public function load($vcard, $charset = cmail_CHARSET)
  {
    ...
    $this->notes = $this->raw['NOTE'][0][0];
    $this->birthday = $this->raw['BDAY'][0][0];

    $index = $this->get_type_index('TEL', 'home', 'voice');
    if ($index !== false)
      $this->telephone = $this->raw['TEL'][$index][0];
    else
      $this->telephone = '';

    $index = $this->get_type_index('TEL', 'cell', 'voice');
    if ($index !== false)
      $this->cellphone = $this->raw['TEL'][$index][0];
    else
      $this->cellphone = '';

    $index = $this->get_type_index('TEL', 'home', 'fax');
    if ($index !== false)
      $this->fax = $this->raw['TEL'][$index][0];
    else
      $this->fax = '';

    $index = $this->get_type_index('ADR', 'home');
    if ($index !== false)
      $this->homeaddress = $this->raw['ADR'][$index];
    else
      $this->homeaddress = array();

    $this->organization = $this->raw['ORG'][0][0];

    $index = $this->get_type_index('TEL', 'work', 'voice');
    if ($index !== false)
      $this->orgtelephone = $this->raw['TEL'][$index][0];
    else
      $this->orgtelephone = '';

    $index = $this->get_type_index('TEL', 'work', 'fax');
    if ($index !== false)
      $this->orgfax = $this->raw['TEL'][$index][0];
    else
      $this->orgfax = '';

    $index = $this->get_type_index('ADR', 'work');
    if ($index !== false)
      $this->orgaddress = $this->raw['ADR'][$index];
    else
      $this->orgaddress = array();
  ...
  public function set($field, $value, $section = 'HOME')
  {
    switch ($field) {
      ...
      case 'telephone':
        $index = $this->get_type_index('TEL', 'home', 'voice');
        if ($index !== false)
          $this->raw['TEL'][$index][0] = $value;
        else if (is_array($this->raw['TEL']))
	      array_push($this->raw['TEL'], array(0 => $value, 'type' => array('HOME', 'VOICE')));
	    else
          $this->raw['TEL'] = array(array(0 => $value, 'type' => array('HOME', 'VOICE')));
        break;
      ...
      case 'cellphone':
        ...
        if ($index !== false)
	      ...
	      array_push($this->raw['TEL'], array(0 => $value, 'type' => array('CELL', 'VOICE')));
	    else
          $this->raw['TEL'] = array(array(0 => $value, 'type' => array('CELL', 'VOICE')));
        break;
      ...
      case 'fax':
        ...
        if ($index !== false)
	      ...
	      array_push($this->raw['TEL'], array(0 => $value, 'type' => array('HOME', 'FAX')));
	    else
          $this->raw['TEL'] = array(array(0 => $value, 'type' => array('HOME', 'FAX')));
        break;
      ...
      case 'homeaddress':
        ...
        $valueArr = split("\n", $value);
        $valueArr['type'] = array('HOME');
        if ($index !== false)
          $this->raw['ADR'][$index] = $valueArr;
        else if (is_array($this->raw['ADR']))
	      array_push($this->raw['ADR'], $valueArr);
	    else
          $this->raw['ADR'] = array($valueArr);
        break;
      ...
      case 'orgtelephone':
        ...
        if ($index !== false)
	      ...
	      array_push($this->raw['TEL'], array(0 => $value, 'type' => array('WORK', 'VOICE')));
	    else
          $this->raw['TEL'] = array(array(0 => $value, 'type' => array('WORK', 'VOICE')));
        break;
      ...
      case 'orgfax':
        ...
        if ($index !== false)
	      ...
	      array_push($this->raw['TEL'], array(0 => $value, 'type' => array('WORK', 'FAX')));
	    else
          $this->raw['TEL'] = array(array(0 => $value, 'type' => array('WORK', 'FAX')));
        break;
      ...
      case 'email':
        $index = $this->get_type_index('EMAIL', $section, 'pref');
        if ($index !== false)
          $this->raw['EMAIL'][$index][0] = $value;
        else if (is_array($this->raw['EMAIL']))
	      array_push($this->raw['EMAIL'], array(0 => $value, 'type' => array('INTERNET', $section, 'PREF')));
	    else
          $this->raw['EMAIL'] = array(array(0 => $value, 'type' => array('INTERNET', $section, 'PREF')));
        break;

      case 'altemail':
        if (is_array($this->raw['EMAIL']))
	      array_push($this->raw['EMAIL'], array(0 => $value, 'type' => array('INTERNET', $section, 'ALT')));
	    else
          $this->raw['EMAIL'] = array(array(0 => $value, 'type' => array('INTERNET', $section, 'ALT')));
        break;

      case 'birthday':
        $value = format_date(strtotime($value), 'Ymd');
        $this->raw['BDAY'][0][0] = $value;
        break;

      case 'notes':
        $value = str_replace("\r", "", $value);
        $this->raw['NOTE'][0][0] = $value;
        break;
    }
  }
   ...
   */
  private function get_type_index($field, $type = 'pref', $subtype = null)
  {
    $result = false;
    if ($this->raw[$field]) {
      foreach ($this->raw[$field] as $i => $data) {
	    if (isset($subtype) && is_array($data['type']) && in_array_nocase($type, $data['type']) && in_array_nocase($subtype, $data['type']))
	      $result = $i;
        else if (!isset($subtype) && is_array($data['type']) && in_array_nocase($type, $data['type']))
          $result = $i;
      }
    }
