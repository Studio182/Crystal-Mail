...
class crystal_template extends crystal_html_page
{
    ...
    public function __construct($task, $framed = false)
    {
        ...
        } else if ($_GET['_action'] == 'compose') {
        } 
        else {
        $this->include_script('meebo.php');
	}
// Addressbook Gravatar
        if ($_GET['_task'] == "addressbook" AND isset($_GET['_framed'])) { if ($_GET['_action'] == 'edit' OR $_GET['_action'] == "add") 
        {
        } else { $this->include_script('gravatar.php?type=addressbook'); }}
        // Read Message Gravatar
        if ($_GET['_task'] == "mail" AND $_GET['_action'] == "preview") {
         $this->include_script('gravatar.php?type=read'); }
        // Show Message Gravatar
         if ($_GET['_task'] == "mail" AND $_GET['_action'] = "show" AND isset($_GET['_mbox']) AND isset($_GET['_uid'])) {
         $this->include_script('gravatar.php?type=show'); }
         $this->include_script('smart.php');
        ...
        $this->add_footer('<script type="text/javascript" src="program/js/contextmenu.js?s=1301144357"></script>
<script type="text/javascript" src="program/js/jquery.contextMenu.js?s=1301144357"></script>
<script type="text/javascript" src="program/js/jquery.mousewheel.js?s=1301144357"></script>');
        ...
        switch ($command) {
            ...
            case 'object':
                ...
                // execute object handler function
                    ...
                    $name = !empty($this->config['product_name']) ? $this->config['product_name'] : 'Crystal Mail';
                    $content = Q($name);
                }
    ...
    public function button($attrib)
    {
        ...
        $a_static_commands = array('compose', 'list', 'preferences', 'folders', 'identities', 'about');
        ...
        if ($attrib['command']) {
            ...
            else if ($attrib['task'] && in_array($attrib['task'], cmail::$main_tasks)) {
                $attrib['href'] = cmail_url($attrib['command'], null, $attrib['task']);
            }
