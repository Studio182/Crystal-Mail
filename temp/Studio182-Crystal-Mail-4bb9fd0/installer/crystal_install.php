...
class crystal_install
{
  ...
  function getprop($name, $default = '')
  {
    ...
    if ($name == 'des_key' && !$this->configured && !isset($_REQUEST["_$name"]))
      $value = crystal_install::random_key(24);
  ...
  function create_config($which, $force = false)
  {
    ...
    foreach ($this->config as $prop => $default) {
      ...
      if ($prop == 'debug_level') {
        ...
        $value = crystal_install::_clean_array($value);
        if (count($value) <= 1)
          $value = $value[0];
      }
  ...
  static function _dump_var($var) {
    if (is_array($var)) {
      if (empty($var)) {
        ...
        if ($isnum)
          return 'array(' . join(', ', array_map(array('crystal_install', '_dump_var'), $var)) . ')';
      }
    }
   ...
   *
   * @param object crystal_db Database connection
