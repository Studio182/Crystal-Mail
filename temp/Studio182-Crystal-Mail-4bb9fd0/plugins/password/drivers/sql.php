...
function password_save($curpass, $passwd)
{
    ...
    if (strpos($sql, '%D') !== FALSE) {
        ...
        if (!($method = $cmail->config->get('password_dovecotpw_method')))
            $method = 'CRAM-MD5';
        $tmpfile = tempnam('/tmp', 'crystalmail-');
    ...
    $sql = str_replace('%l', $db->quote($cmail->user->get_username('local'), 'text'), $sql);
    $sql = str_replace('%d', $db->quote($cmail->user->get_username('domain'), 'text'), $sql);
